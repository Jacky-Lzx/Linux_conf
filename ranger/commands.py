...
class latex_clean(Command):
    """
    : Clean up the auxilary files generated by latex in current directory
    """
...
class latex_clean_all(Command):
    """
    : Clean up all the auxilary files generated by latex in subdirectories (one depth).
    """
    ...
    def execute(self):
        ...
        for _, dirs, _ in os.walk(self.fm.thisdir.path):
            # print(f"dirs:{dirs}")
            for direct in dirs:
                for root, _, paths in os.walk(self.fm.thisdir.path + "/" + direct):
                    for path in paths:
                        for string in remove_str:
                            if path.endswith(string):
                                shutil.move(root + "/" + path, "removed/" + path)
